...
public class SqliteDbMigrator : IDbMigrator
{
    ...
    public SqliteDbMigrator(IDbContextFactory<TrackitDbContext> dbContextFactory, DALOptions dalOptions)
    {
    ...
    {
        await using TrackitDbContext dbContext = await _dbContextFactory.CreateDbContextAsync(cancellationToken);
